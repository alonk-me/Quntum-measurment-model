...
def run_trajectory(N: int, epsilon: float, omega: float = 0.0, rng: Optional[np.random.Generator] = None) -> TrajectoryResult:
    ...
    omega : float, optional
        Frequency of coherent rotation about the y‑axis.  The qubit
        rotates by an angle ``omega/2`` before each measurement.  The
    ...
    # Precompute rotation coefficients for the Hamiltonian step
    half_angle = 0.5 * omega
    c = cos(half_angle)
    s = sin(half_angle)
    ...
    for _ in range(N):
        ...
        # Coherent rotation generated by H = omega * sigma_y / 2
        # Apply rotation U = cos(omega/2)*I - i*sin(omega/2)*sigma_y
        # which in the computational basis has real entries (see module docstring).
        rot_alpha = c * alpha - s * beta
        rot_beta = s * alpha + c * beta
        alpha, beta = rot_alpha, rot_beta
...
def simulate_Q_distribution(num_traj: int, N: int, epsilon: float, omega: float = 0.0, seed: Optional[int] = None) -> np.ndarray:
    ...
    for i in tqdm(range(num_traj)):
        ...
        res = run_trajectory(N, epsilon, omega, rng)
        Q_values[i] = res.Q
    return Q_values
