...
class SSEWavefunctionSimulator:
    ...
    J : float
        ...
        When J≠0, adds Hamiltonian H = J σ_y to the evolution.
    initial_state : str
    ...
    def _measurement_update(self, psi: np.ndarray) -> Tuple[np.ndarray, int, float]:
        ...
        # Sample measurement outcome: unbiased coin flip ξ ∈ {+1, -1}
        xi = 1 if self.rng.random() < 0.5 else -1
    ...
    def simulate_trajectory(self) -> Tuple[float, np.ndarray, np.ndarray]:
        ...
        for i in range(self.N_steps):
            ...
            Q += self.Q_sum_stratonovich(z_before, z_after, xi)
            # Accumulate entropy production using the exact per-step log rule
            # ΔQ = log((1 + ε ξ z_before) / (1 − ε ξ z_after))
            # Q += self.Q_sum_log(z_before, z_after, xi)
            # Alternative Stratonovich-like accumulation:
            # ΔQ = 2 ε ξ (z_before + z_after)/2 + 2 ε² z_before (z_before + z_after)/2
            # incerement part of Q
            # Q += self.Q_sum_stratonovich_v2(z_before, z_after, xi)
        # boundary term: (ε^2 / 2) · (z_N^2 − z_0^2)
        # Q += self.Q_stratonovich_v2_boundary(z_trajectory[0], z_trajectory[-1])
    ...
    def Q_sum_stratonovich(
        self,
        z_before: float,
        z_after: float,
        xi: int,
    ) -> float:
        """Per-step ΔQ using the Stratonovich-like rule currently in comments.
        ΔQ = 2 ε ξ (z_before + z_after)/2 + 2 ε² z_before (z_before + z_after)/2
        """
        first_order = 2.0 * self.epsilon * float(xi) * 0.5 * (float(z_before) + float(z_after))
        second_order = 2.0 * (self.epsilon ** 2) * float(z_before) * 0.5 * (float(z_before) + float(z_after))
        return float(first_order + second_order)
    ...
    def Q_sum_log(
        ...
        """Per-step ΔQ using the exact log-likelihood increment.
        ΔQ = log((1 + ε ξ z_before) / (1 − ε ξ z_after)).
        """
        num = 1.0 + self.epsilon * float(xi) * float(z_before)
        den = 1.0 - self.epsilon * float(xi) * float(z_after)
        if clip is not None and clip > 0.0:
            num = max(num, clip)
            den = max(den, clip)
        return float(np.log(num / den))

    def Q_sum_stratonovich_v2(
        self,
        z_before: float,
        z_after: float,
        xi: int,
    ) -> float:
        """Per-step ΔQ from the Stratonovich midpoint term in the expansion.
        Implements only the midpoint contribution
            ΔQ_mid = 2 ε ξ · (z_before + z_after)/2
        To reproduce the full expression from the derivation,
        add the boundary correction once per trajectory after the loop:
            Q_boundary = (ε^2 / 2) · (z_N^2 − z_0^2)
        so that
            Q ≈ Σ_i ΔQ_mid(i) + Q_boundary + O(N ε^3).
        """
        return float(2.0 * self.epsilon * float(xi) * 0.5 * (float(z_before) + float(z_after)))


    def Q_stratonovich_v2_boundary(self, z0: float, zN: float) -> float:
        """One-time boundary correction for the v2 Stratonovich expansion.
        Q_boundary = (ε^2 / 2) · (z_N^2 − z_0^2)
        """
        return 0.5 * (self.epsilon ** 2) * (float(zN) ** 2 - float(z0) ** 2)
